...
#     # Draw rectangles around detected faces and crop the frame to the first detected face
#     if len(faces) > 0:
...
#         cv2.putText(frame, "Human", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)  # Add "Human" text
#         cropped_face = frame[y:y + h, x:x + w]  # Crop the frame to the face region
#     #     cv2.imshow('Cropped Face', cropped_face)  # Display the cropped face
#     # else:
#         cv2.imshow('Webcam', frame)  # Display the original frame if no face is detected
